...
【価格の選択ルール（重要）】:
1. 「税込」「税込価格」「(税込)」「[税込」などのラベルが付いた価格を最優先で選択する
2. 「本体価格」「本体」とラベルされた価格は税抜き価格なので絶対に選ばない
3. 税込価格が「149.04円」のように小数の場合は整数部分（149）のみを返す
4. 税込価格が明示されていない場合:
   - 食品・飲料の場合は本体価格 × 1.08 を計算して整数に丸める
   - それ以外の場合は本体価格 × 1.10 を計算して整数に丸める
5. 価格が1種類しかない場合はその価格をそのまま返す
