---
name: fix-issue
description: GitHub Issueを自動解決するワークフロー。Working Documents読み込み→Issue説明→計画→実装→動作確認→要件確認→検証→独立レビュー→PR作成・自動マージまで。Issue番号を引数に取る。「/fix-issue 123」のように使用。トピックブランチ必須、main直接コミット禁止。
---
...
```
Working読込 → Issue確認 → 説明 → 計画 → 実装 → 動作確認 → 要件確認 → 検証 → PR作成・レビュー → 自動マージ → クリーンアップ
                          🔹①     🔹②                       🔹③
```
...
1. **テストファーストで実装**
   - testing.md を読み込み → テスト設計のインプット
   - 失敗テストを作成 → テスト合格する最小実装 → 改善（必要な場合のみ）
   - テストは `flutter test` で実行

2. **Flutter/Dart固有の注意点**
   - `flutter analyze` でLintエラーがないことを確認
   - `kIsWeb` によるプラットフォーム分岐に注意
   - Providerパターンに従った状態管理
   - `data_provider.dart` は1500行超の大規模ファイル。変更時は影響範囲を慎重に確認

#### テスト対象外の場合（docs, chore, ビジュアル調整のみ）

Claude Code標準ツール(Edit/Write)でそのまま実装。

#### 共通

**tasklist.mdを逐次更新**: 完了したタスクは `[x]` に変更。
...
1. **手動確認（推奨）** - ユーザーが自分で端末/エミュレーターで確認
2. **スキップ** - 動作確認を省略（テストで十分と判断した場合）
...
- **このフェーズでは `flutter analyze` / `flutter test` を実行しない**
...
1. **コミット・プッシュ**
   ...
   **① 実装外の変更がある場合（chore コミットを先に作成）**
   ...
   対象例: `.claude/skills/`, `.gitignore`, `CLAUDE.md`, `docs/`
   ...
   **② 実装コミット**
   ...
   両方のコミットを同じPR（同じブランチ）に含める。別ブランチは不要。

2. **PR作成**
   ```bash
   # 一時ファイルはリポジトリルートに相対パスで作成（Windows互換）
...
3. **コードレビュー（選択式）**
   ...
   1. **フルレビュー** - code-reviewプラグインで多角的レビュー（大規模変更・重要機能向け）
   2. **スキップ** - コードレビューを省略（小規模修正等）
...
4. **レビュー結果の処理**
   - **問題なし / スキップ** → Phase 10（自動マージ）へ
   - **問題あり** → 修正 → コミット → プッシュ → 再度code-reviewを実行（ループ）

---

## Phase 10: 最終更新・自動マージ・クリーンアップ（全自動）
...
### 2. Working更新のコミット・プッシュ（更新があった場合）

```bash
git add docs/
git commit -m "docs(#<Issue番号>): Working Documents更新"
git push
```
...
if [ "$STATE" = "MERGED" ]; then
  ...
  echo "マージがまだ完了していません（state: $STATE）"
